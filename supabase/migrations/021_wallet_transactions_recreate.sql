-- Reset wallet_transactions to the canonical admin-audit schema.
-- Use when legacy columns (amount, transaction_type, etc.) block admin_adjust_wallet.
-- Safe to re-run: skips rename if canonical table already exists.
...
DO $$
BEGIN
  -- Legacy table: has transaction_type OR lacks delta — replace with canonical schema.
  IF EXISTS (
    ...
    IF EXISTS (
      SELECT 1 FROM information_schema.tables
      WHERE table_schema = 'public' AND table_name = 'wallet_transactions_legacy'
    ) THEN
      DROP TABLE public.wallet_transactions_legacy;
    END IF;
